...
import { Mail, ArrowLeft, KeyRound, Lock, CheckCircle, Eye, EyeOff } from "lucide-react";
...
const ForgotPasswordPage = () => {
  ...
  const handleRequestPin = async (e) => {
    ...
    try {
      ...
      setError(err?.response?.data?.message || "Failed to send PIN. Please check your email.");
    } finally {
      setLoading(false);
    }
  };
          ...
          <div className="login-card">
            ...
            <div className="login-header">
                <div className="neu-icon">
                    <div className="icon-inner">
                        ...
                        {step === 3 && <Lock size={24} />}
                        {step === 4 && <CheckCircle size={24} className="text-green-500"/>}
                    </div>
                </div>
                <h2>
                ...
                <form className="login-form" onSubmit={handleRequestPin}>
                    <div className={`form-group ${error ? 'error' : ''}`}>
                        <div className="input-group neu-input">
                         ...
                         {error && <span className="error-message show" style={{display: 'block'}}>{error}</span>}
                    </div>
            ...
            {step === 2 && (
                <form className="login-form" onSubmit={handleVerifyPin}>
                    <div className={`form-group ${error ? 'error' : ''}`}>
                        <div className="input-group neu-input">
                            <input 
                                ...
                                disabled={loading}
                            />
                            <label className="text-center w-full left-0">Enter 6-digit PIN</label>
                        </div>
                         {error && <span className="error-message show" style={{display: 'block'}}>{error}</span>}
                    </div>
                    <button type="submit" className={`neu-button login-btn ${loading ? 'loading' : ''}`} disabled={loading}>
                    ...
                    <div className="text-center mt-4">
                        <button type="button" onClick={() => setStep(1)} className="text-sm text-gray-500 hover:text-blue-600">
                ...
                <form className="login-form" onSubmit={handleResetPassword}>
                    <div className={`form-group ${error ? 'error' : ''}`}>
                        <div className="input-group neu-input password-group">
                            ...
                            <button type="button" className="password-toggle neu-toggle" onClick={() => setShowPassword(!showPassword)}>
            ...
            {step === 4 && (
                <div className="text-center py-6">
                    ...
                    <Link to="/login" className="neu-button login-btn inline-flex items-center justify-center text-white no-underline">
            ...
            {step !== 4 && (
                ...
                <div className="signup-link">
                    <p>
                        <Link to="/login" className="flex items-center justify-center font-bold text-gray-700 hover:text-blue-600 transition-colors">
